# Analytics Guide

Analytics endpoints are shop-scoped and support date ranges.

## Dashboards

- Funnel analytics: inbound → resolved product → resolved variant → draft/order/payment.
- Post revenue: paid orders and revenue by Instagram post.
- Lost/unavailable demand: unresolved or unavailable color/size/variant demand.
- Operator and agent performance: handoff, response, and automation metrics.

## Interpretation rules

- Revenue metrics should be based on paid orders, not only message volume.
- Empty states are expected for new shops or narrow date ranges.
- Use paid-order revenue for pilot and production readiness decisions.
